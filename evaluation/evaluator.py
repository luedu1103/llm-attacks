...
def _build_prompt(example: dict[str, Any]) -> str:
    ...
    # Construir el rango de letras válidas dinámicamente
    valid_letters = ", ".join(
        _OPTION_LETTERS[: len(options)]
    )  # "A, B, C, D" si hay 4 opciones
    answer_instruction = (
        f"Responde ÚNICAMENTE con la letra de la opción correcta ({valid_letters})."
    )
...
def evaluate_dataset(
    ...
    for i, example in enumerate(
        tqdm(
            remaining, desc=desc, unit="ítem", initial=already_done, total=len(dataset)
        )
    ):
