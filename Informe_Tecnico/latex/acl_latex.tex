...
Los modelos de lenguaje basados en transformadores han alcanzado rendimiento
sobresaliente en una amplia variedad de tareas de procesamiento del lenguaje
natural \citep{devlin-etal-2019-bert}.
Sin embargo, estudios recientes muestran que buena parte de ese rendimiento se
apoya en \textit{atajos léxicos}: correlaciones estadísticas superficiales entre
palabras del enunciado y la respuesta correcta, en lugar de una comprensión
semántica profunda \citep{mccoy-etal-2019-right}.

Una consecuencia práctica es que pequeñas perturbaciones del texto —reemplazar
una palabra por un sinónimo o eliminar un conector causal— pueden provocar un
cambio drástico en la predicción del modelo incluso cuando el significado es
idéntico.
Este fenómeno compromete la fiabilidad de los sistemas en contextos reales y
dificulta su certificación para aplicaciones de alto impacto.

El problema se agudiza en español, idioma con menor representación en los
\textit{benchmarks} de robustez existentes frente al inglés.
La mayoría de los trabajos sobre adversarialidad léxica en PLN \citep{jia-liang-2017-adversarial,jin-etal-2020-bert,gao-etal-2018-black}
se centran en inglés, lo que limita la evaluación de modelos desplegados en
contextos hispanohablantes.

En este trabajo describimos el diseño e implementación de un \textbf{marco de
evaluación de robustez léxica} para tareas de razonamiento verbal en español,
cuyos aportes principales son:

\begin{enumerate}
    \item Cuatro módulos de perturbación controlada (sinónimos, parafraseo, pares
          mínimos y eliminación de atajos) con una interfaz unificada y soporte
          de paralelismo.
    \item Un \textit{pipeline} de generación de datos perturbados con sistema de
          \textit{checkpoints} para tolerancia a fallos.
    \item Validación semántica automática mediante \textit{embeddings} multilingües
          que garantiza la preservación del significado en cada ejemplo generado.
    \item Un conjunto de datos de 13\,829 ejemplos en español con siete tipos de
          tarea, listos para ser evaluados.
    \item Un módulo de métricas (precisión, $\Delta$\,precisión y tasa de cambio)
          listo para conectarse a cualquier modelo evaluador.
\end{enumerate}
